...
Lastly, `MRFcov` has the capability to account for possible spatial autocorrelation when estimating interaction parameters. To do this, we incorporate functions from the `mgcv` package to include unpenalized Gaussian process spatial regression splines as fixed covariates in each node-wise regression. The user must supply a two-column `data.frame` called `coords`, which will ideally contain Latitude and Longitude values for each row in `data`. We don't have these coordinates for the `Bird.parasites` dataset, so we will instead create some fake coordinates to showcase the model
```{r eval = FALSE}
