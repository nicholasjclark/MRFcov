...
bootstrap_MRF <- function(data, n_bootstraps, n_its, sample_seed, min_lambda1,
  ...
  if(missing(fixed_lambda)){
    warning('fixed_lambda not provided. Using cross-validated optimisation by default,
  ...
  if(!fixed_lambda){
    #If using node-optimised cross-validation, lambda1_seq doesn't matter (just needs to be a sequence for
    #running models repeatedly, if preferred. Uses n_its to define the length)
    lambda1_seq <- seq_len(n_its)
